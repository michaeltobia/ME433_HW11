...
/*IMU data storage*/
unsigned char data_store[14];   // raw IMU data
signed short data_conv[7];      // converted IMU data
int j = 0;                      // counter for converting IMU data
...
void APP_Initialize(void) {
    ...
    TRISBbits.TRISB8 = 0;   // B8 as output (IMU whoami LED)
...
//    int8_t dir_table[] = {-4, -4, -4, 0, 4, 4, 4, 0};
    static uint8_t inc = 0;     // HID delay counter
    static int8_t mouse_x = 0;
    static int8_t mouse_y = 0; // data stores for mouse command conversion
    static char whoami_check = 0;
    ...
    switch (appData.state) {
        ...
        case APP_STATE_MOUSE_EMULATE:
            whoami_check = getReg(0x0F);
            if(whoami_check == 0x69){
                LATBINV = 0x100;
            }
            I2C_read_multiple(SLAVE_ADDR, 0x20, data_store, 14);
            for(j=0;j<7;j++){
                data_conv[j] = (data_store[j*2+1] << 8) | data_store[j*2];
            }
...
//            if (movement_length > 50) {
            ...
            if(inc < 20){
                ...
                mouse_x = (int8_t) ((data_conv[6])/4000);
                mouse_y = (int8_t) ((data_conv[4])/4000);
